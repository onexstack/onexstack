...
func flagsUsages(f *flag.FlagSet) (string, error) {
	...
	wrapLimit, err := getWordWrapperLimit()
	if err != nil {
		wrapLimit = 0
	}
...
// union returns a new FlagSet containing the flags of l and, for any name not
// already present, the flags of r. Used to include inherited parent flags in a
// subcommand's help output.
func union(l, r *flag.FlagSet) *flag.FlagSet {
	f := flag.NewFlagSet("union", flag.ContinueOnError)
	l.VisitAll(func(flag *flag.Flag) { f.AddFlag(flag) })
